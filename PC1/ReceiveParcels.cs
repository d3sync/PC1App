...
namespace PC1
{
    public partial class ReceiveParcels : Form
    {
            ...
            try
            {
                ...
                {//ParcelBarcode,InvBarcode,VoucherBarcode,Name,Address,Price,Driver,regDate
                    ...
                    var lvi = new ListViewItem(
                        new string[] { item.id.ToString(), item.ParcelBarcode, item.InvBarcode, item.VoucherBarcode, item.Name, item.Address, item.Price, item.Phone, item.Phone2, item.Comments, item.ArrivalDate }
                    );
                    switch (item.IsCompleted)
                    {
                        case 0: lvi.BackColor = Color.White; break;
                        case 1:
        ...
        private void txtPrice_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if ((e.KeyChar is ',' or '.') && ((sender as TextBox).Text.IndexOf(',') > -1))
            {
                e.Handled = true;
            }
        }
        ...
        private void cmlv_Edit_Click(object sender, EventArgs e)
        {
            ...
            {
                var res = _context.ReceivedParcels.FirstOrDefault(m => m.id == Guid.Parse(listView1.SelectedItems[0].SubItems[0].Text));
                EditID = res.id;
                EditOn = true;
